...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('selectDateContent').classList.remove('d-none');
...
function updateSummary() {
  ...
  if (selectedYear === null) {
    summaryText.textContent = 'Choose year, month, and day below';
  } else if (selectedMonth === null) {
    summaryText.textContent = `${selectedYear} — Select a month`;
  } else if (selectedDay === null) {
    summaryText.textContent = `${MONTH_NAMES[selectedMonth]} ${selectedYear} — Select a day`;
  } else {
    const dateStr = `${selectedYear}-${String(selectedMonth + 1).padStart(2, '0')}-${String(selectedDay).padStart(2, '0')}`;
    const dateObj = parseIsoDate(dateStr);
    summaryText.textContent = formatDisplayDateWithDay(dateObj);
  }
}
